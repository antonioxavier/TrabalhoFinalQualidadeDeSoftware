...
public class SystemInitializer {
    ...
    public static StringBuilder generateString() {
    	 final StringBuilder sb1 = new StringBuilder(PASSWORD_LENGTH);
         return sb1;
    }
    ...
    public static String generateRandom() {
        ...
        for (int i = 0; i < PASSWORD_LENGTH; i++) {
            if (random.nextBoolean()) {
            	 generateString().append(random.nextInt(10));
            } else {
                char c = LETTERS.charAt(random.nextInt(LETTERS.length()));
                generateString().append(random.nextBoolean() ? c : Character.toUpperCase(c));
            }
